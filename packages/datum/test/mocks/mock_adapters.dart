...
class MockLocalAdapter<T extends DatumEntityInterface> implements LocalAdapter<T> {
  ...
  final Map<Type, LocalAdapter<DatumEntityInterface>>? relatedAdapters;
  ...
  Stream<List<T>>? watchAll({String? userId, bool? includeInitialData}) {
    ...
    final updateStream = stream
        .where((event) => userId == null || event.userId == userId || event.userId == '__REFRESH_STREAMS__' // Special refresh marker
            )

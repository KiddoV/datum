    ...
    test('onConflictDetected and onConflictResolved are called', () async {
      ...
      verify(
        () => mockGlobalObserver.onConflictDetected(
          any(that: predicate<DatumEntityInterface>((e) => e.id == local.id)),
          any(that: predicate<DatumEntityInterface>((e) => e.id == remote.id)),
          any(that: isA<DatumConflictContext>()),
        ),
      ).called(1);
      ...
      verify(
        () => mockGlobalObserver.onConflictResolved(
          any(
            that: isA<DatumConflictResolution<DatumEntityInterface>>().having(
              (r) => r.strategy,
              'strategy',
